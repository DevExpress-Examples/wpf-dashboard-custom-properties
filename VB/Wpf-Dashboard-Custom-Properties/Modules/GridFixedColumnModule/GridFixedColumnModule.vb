...
Namespace Wpf_Dashboard_Custom_Properties
	...
	Public Class GridFixedColumnsConverter
		...
		Private Function FindColumn(ByVal fieldName As String, ByVal columns As DevExpress.DashboardCommon.GridColumnCollection) As DevExpress.DashboardCommon.GridColumnBase
			Dim dimensionColumn As GridDimensionColumn = columns.OfType(Of GridDimensionColumn)().FirstOrDefault(Function(column) column.Dimension.UniqueId = fieldName)
			If dimensionColumn IsNot Nothing Then
				Return dimensionColumn
			End If
			Dim measureColumn As GridMeasureColumn = columns.OfType(Of GridMeasureColumn)().FirstOrDefault(Function(column) column.Measure.UniqueId = fieldName)
			If measureColumn IsNot Nothing Then
				Return measureColumn
			End If
			Dim hyperlinkColumn As GridHyperlinkColumn = columns.OfType(Of GridHyperlinkColumn)().FirstOrDefault(Function(column) column.DisplayValue.UniqueId = fieldName)
			If hyperlinkColumn IsNot Nothing Then
				Return hyperlinkColumn
			End If
			Dim deltaColumn As GridDeltaColumn = columns.OfType(Of GridDeltaColumn)().FirstOrDefault(Function(column) (If(column.ActualValue, column.TargetValue)).UniqueId = fieldName)
			If deltaColumn IsNot Nothing Then
				Return deltaColumn
			End If
			Dim sparklineColumn As GridSparklineColumn = columns.OfType(Of GridSparklineColumn)().FirstOrDefault(Function(column) column.Measure.UniqueId = fieldName)
